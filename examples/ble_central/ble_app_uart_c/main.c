...
#define APPL_LOG(...)                   /**< Debug logger macro that will be used in this file to do logging of debug information over UART. */
                                                                
#define SCAN_INTERVAL           0x00A0                          /**< Determines scan interval in units of 0.625 millisecond. */
#define SCAN_WINDOW             0x0050                          /**< Determines scan window in units of 0.625 millisecond. */
#define SCAN_ACTIVE             1                               /**< If 1, performe active scanning (scan requests). */
#define SCAN_SELECTIVE          0                               /**< If 1, ignore unknown devices (non whitelisted). */
#define SCAN_TIMEOUT            0x0010                          /**< */
...
uint8_t clintmacaddr[6];
uint8_t myconcetbit=0;
/**
...
void uart_event_handle(app_uart_evt_t * p_event)
{
    ...
    switch (p_event->evt_type)
    {
        ...
        case APP_UART_DATA_READY:
            UNUSED_VARIABLE(app_uart_get(&res));
					...
					if(USART_RX_STA&0x4000)
						{
							if(res==0x0a)USART_RX_STA|=0x8000;
								...
								{
									USART_RX_STA=0;
									data_array[index]=0x0d;
									index++;
									data_array[index]=res;
									index++;
									if(index >= (BLE_NUS_MAX_DATA_LEN))USART_RX_STA|=0x8000;
								}
						}
					else
						{
			...
			if(USART_RX_STA&0x8000)
				{
					 USART_RX_STA=0;
					 if(data_array[0]==0xff&&data_array[1]==0xff)
					 	{
					 		for(i=0;i<6;i++)clintmacaddr[i]=data_array[2+i];
							printf("SET OK\r\n");
							if(myconcetbit==1)
								{
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_ADV_REPORT:
        {
			...
			/*printf("Scanf sub mac: %02x:%02x:%02x:%02x:%02x:%02x\r\n",
            ...
            if(clintmacaddr[0]== p_adv_report->peer_addr.addr[0]&&clintmacaddr[1]== p_adv_report->peer_addr.addr[1])
            	{
            		if(clintmacaddr[2]== p_adv_report->peer_addr.addr[2]&&clintmacaddr[3]== p_adv_report->peer_addr.addr[3])
            			{
            				if(clintmacaddr[4]== p_adv_report->peer_addr.addr[4]&&clintmacaddr[5]== p_adv_report->peer_addr.addr[5])
            					{
...
int main(void)
{
    ...
    APP_ERROR_CHECK(err_code);
    for(i=0;i<6;i++)clintmacaddr[i]=0xff;
    nus_c_init();
